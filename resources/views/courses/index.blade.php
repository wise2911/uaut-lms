           ...
           <div id="mainContent" class="ml-0 lg:ml-64 transition-all duration-300 flex flex-col min-h-screen">
               ...
               <main class="container mx-auto px-6 py-8 flex-grow">
                   ...
                   <div class="bg-white rounded-2xl shadow-lg p-8 mb-8">
                       ...
                       <form method="GET" action="{{ route('courses.index') }}" class="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4">
                   ...
                   <div class="mb-12">
                       ...
                       @else
                           <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                               @foreach ($courses as $course)
                                   <div class="course-card bg-white rounded-2xl shadow-md overflow-hidden">
                                       <img src="{{ $course->thumbnail ? asset('storage/' . $course->thumbnail) : asset('img/placeholder.jpg') }}" alt="{{ $course->title }} Thumbnail" class="w-full h-48 object-cover" onerror="this.src='https://via.placeholder.com/400x200'">
                                       <div class="p-5">
                                           <div class="flex justify-between items-start mb-2">
                                               <h3 class="text-lg font-semibold text-gray-800 line-clamp-2">{{ $course->title }}</h3>
                                               <span class="text-xs bg-primary-100 text-primary-800 px-2 py-1 rounded-full">{{ $course->department ?? 'General' }}</span>
                                           </div>
                                           <p class="text-gray-600 text-sm mb-3 line-clamp-3">{{ $course->short_description ?? 'No description available' }}</p>
                                           <p class="text-gray-500 text-xs mb-3">By {{ $course->instructor_name ?? 'Unknown Instructor' }}</p>
                                           <p class="text-gray-500 text-sm mb-3">Price: ${{ number_format($course->price, 2) }}</p>
                           ...
                           <div class="mt-8">
                               {{ $courses->appends(['search' => request('search'), 'department' => request('department')])->links('vendor.pagination.tailwind') }}
                           </div>
                       @endif
                   </div>
               </main>
